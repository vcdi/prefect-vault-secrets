...
@pytest.mark.parametrize('vault_var', [
    'vault_addr',
    'VAULT_ADDR'
])
def test_vault_addr_from_env_var(vault_var, server_api):
    ...
    with pytest.raises(AttributeError,
                       match=r"'Context' object has no attribute 'parameters'"):
        task = VaultSecretTestTask('fake-no-vault-addr-secret')
        task.run()
...
@pytest.mark.parametrize('vault_creds', [
...
def test_vault_secret_lookup(vault_creds, server_api):
    ...
    with prefect.context(
        ...
        Parameter('vault.credentials', default="fake-vault-creds")
        task = VaultSecretTestTask('secret/fake-path/fake-key')
        out = task.run()
        assert out == "fake-value"
