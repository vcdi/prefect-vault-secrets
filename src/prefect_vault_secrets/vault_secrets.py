...
class VaultSecret(SecretBase):
    ...
    def _get_vault_secret(self, name):
        ...
        Raises:
            ...
            - PermissionError: unable to authenticate or attempting an
        ...
        self.logger.info('passed vault authentication check')
        ...
        if m is None:
            raise KeyError(
                f'Invalid secret path: {name}.  Expected: "<mount_point>/<path>/<key>"'
            )
        ...
        try:
            ...
            value = vault_secret['data']['data'][vault_path['key']]
        except KeyError as exc:
            raise KeyError(f"Key not found: {vault_path['key']}") from exc
        except hvac.exceptions.Forbidden:
            raise PermissionError(f"Access forbidden: {vault_path['key']}")
        return value
    ...
    def run(self, name: str = None):
        ...
        or `False`.
        Args:
            - name (str, optional): the name of the underlying Secret to retrieve.
